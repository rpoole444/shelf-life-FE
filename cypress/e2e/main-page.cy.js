describe("example to-do app", () => {
  beforeEach(() => {
    cy.intercept("GET", "http://localhost:3001/api/v1/books", {
      fixture: "library.json",
    });
    cy.intercept("GET", "http://localhost:3001/api/v1/books/9780385546027", {
      fixture: "bookDetails.json",
    });
  ...
  it('Should display a title and cards with books that have a title, author, recommendation, "learn More" button, and book cover ', () => {
    ...
    cy.get(".book-cover").should("have.length", 3);
    cy.get(".learn-more-btn").should("have.length", 3);
  ...
  it("Should be able to click a book and visit the book details ", () => {
    ...
    cy.url("http://localhost:3000/").should(
      "include",
      "9780385546027/selectedBook"
    );
  });

  it('Should display a functional route called "Your Favorites", when clicked it takes you to the favorites window. ', () => {
    cy.get("nav").contains("Your Favorites");
